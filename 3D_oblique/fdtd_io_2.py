...
def run(outdir, quiet=True, cache=True, **kw):
    ...
    t0, start = time.perf_counter(), time.strftime("%Y-%m-%d %H:%M:%S")
    ...
    with open(argfile, "w") as fh:
        json.dump(argv, fh)
    with open(os.path.join(outdir, "timing.json"), "w") as fh:
        json.dump(dict(start=start, end=time.strftime("%Y-%m-%d %H:%M:%S"), wall_s=wall,
                       host_cpus=os.cpu_count(), omp_threads=os.environ.get("OMP_NUM_THREADS", "default")), fh)
    if not quiet:
...
def load_timing(outdir):
    """Wall time of the run: timing.json (Python-side, float seconds) if present, else meta runtime_s (integer s)."""
    p = os.path.join(outdir, "timing.json")
    if os.path.exists(p):
        with open(p) as fh:
            return json.load(fh)
    return dict(wall_s=float(load_meta(outdir)["runtime_s"]), start=None, end=None)
