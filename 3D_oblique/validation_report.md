...
程式在啟動時檢查 $0<Q\le1$（$Q\le0$：該模態在此網格上為消逝波；$Q>1$：無實數解），不合格直接報錯
結束，不產生 NaN。取正根，代表往 $+y$ 傳播。另外檢查連續傳播條件 $|k_t|<(1-0.05)\,n\,k_0$（保留 5% 裕度，
避免掠射角附近群速度趨近 0）。
...
3. **阻抗（對 SPEC 的更正）**：$|\mathbf H_0|/|\mathbf E_0|=|\tilde{\mathbf K}|/(\mu_0\tilde\omega)$，而色散關係正是
   ...
   $$\frac{|\mathbf H_0|}{|\mathbf E_0|}=\frac{1}{\mu_0 c_m}=\frac{1}{\eta}\quad\text{（恆等，非近似）}.$$
...
**區域劃分**：總場（TF）＝ $y\ge y_0$ 的所有節點；散射場（SF）＝ $y<y_0$。因此
...
| 6 | PML 反射 | 無閉式；$R_{\rm dB}=20\log_{10}(\lvert B\rvert/\lvert A\rvert)$，$B$＝SF 區穩態反向平面波振幅，目標 $<-40$ dB |
| 7 | 穩定性 | 關源後總能量不增、衰減至 $<10^{-8}\times$ 峰值 |
...
| $E_x,E_z$ | $j<j_1$ / $j>j_1$ | 1 / $n^2$ | 整個對偶胞在單一介質 |
| $E_y$（法向） | $j+\frac12<j_1$ / $>j_1$ | 1 / $n^2$ | 對偶胞 $[j,j+1]\Delta$ 整個在單一介質，無需調和平均（若介面切過 $E_y$ 才需 $\langle\varepsilon^{-1}\rangle^{-1}$，因法向 $D$ 連續、串聯電容） |
| PML 內 | $j>N_y-N_{\rm pml}$ | $n^2$，$\sigma_{\max}$ 除以 $n$ | 介質延伸進遠端 PML（SPEC 要求），見 §7 |
